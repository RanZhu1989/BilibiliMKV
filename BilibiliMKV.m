...
for i = 1:length(subfolders)
    ...
    for j = 1:length(subsubfolders)
        subsubfolder_path = fullfile(subfolder_path, subsubfolders(j).name);         % 获取子文件夹中的子文件夹路径
        ...
        if isfield(entry_file, 'quality_pithy_description')
            quality_pithy_description = entry_file.quality_pithy_description; % 可能有分辨率不存在的问题
        else
            quality_pithy_description = '未知分辨率';
        end
        if isfield(entry_file, 'owner_name')
            owner_name =  regexprep(entry_file.owner_name, '[^\w]+', '');% % 可能有UP主信息丢失问题. 需要加入正则表达式去掉标点 否则奇怪的UP主的名字会出错
        else
            owner_name = '未知UP主';
        end
        entry_content = ['[分P=' part ']_' title '_' quality_pithy_description '_UP主=' owner_name '_BVid=' num2str(entry_file.bvid)]; % 将当前子文件夹中子文件夹中的audio.m4s和video.m4s重命名为{视频标题+UP主姓名+分辨率+AV号.mp3和对应的*.mp4
       ...
       command_mkvmerge = sprintf('mkvmerge -o %s "%s" "%s"', fullfile(output_path, [entry_content, '.mkv']), new_audio_file_path, new_video_file_path);  % 调用mkvmerge将MP3和MP4合并为MKV文件
